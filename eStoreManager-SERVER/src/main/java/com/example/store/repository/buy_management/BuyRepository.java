...
public interface BuyRepository extends JpaRepository<Buy, Long> {
    ...
    @Query("SELECT e from Buy e WHERE e.user_id = :id AND e.createdAt >= STR_TO_DATE(:start, '%Y-%m-%d %H:%i:%s') AND e.createdAt <= STR_TO_DATE(:end, '%Y-%m-%d %H:%i:%s') ")
    Page<Buy> findByUserIdAndDate(Long id, String start, String end, Pageable pageable);

    @Query("SELECT e from Buy e WHERE e.user_id = :id")
    Page<Buy> findByUserId(Long id, Pageable pageable);

    @Query("SELECT e from Buy e WHERE e.createdAt >= STR_TO_DATE(:start, '%Y-%m-%d %H:%i:%s') AND e.createdAt <= STR_TO_DATE(:end, '%Y-%m-%d %H:%i:%s') ")
