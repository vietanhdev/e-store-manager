...
public interface ProductRepository extends JpaRepository<Product, Long>{
    ...
    @Query("SELECT e FROM Product e WHERE e.name LIKE %:name% AND e.unit LIKE %:unit% AND e.barcode LIKE %:barcode% AND ( e.name LIKE %:value% OR e.unit LIKE %:value% OR e.barcode LIKE %:value% )")
