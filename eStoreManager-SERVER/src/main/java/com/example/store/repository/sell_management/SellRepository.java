...
public interface SellRepository extends JpaRepository<Sell, Long> {
    ...
    @Query("SELECT e from Sell e WHERE e.user_id = :user_id AND e.customer_id = :customer_id AND e.createdAt >= STR_TO_DATE(:start, '%Y-%m-%d %H:%i:%s') AND e.createdAt <= STR_TO_DATE(:end, '%Y-%m-%d %H:%i:%s') ")
    Page<Sell> findByUserIdAndCustomerIdAndDate(Long user_id, Long customer_id, String start, String end, Pageable pageable);

    @Query("SELECT e from Sell e WHERE e.user_id = :id")
    Page<Sell> findByUserId(Long id, Pageable pageable);

    @Query("SELECT e from Sell e WHERE e.customer_id = :id")
    Page<Sell> findByCustomerId(Long id, Pageable pageable);

    @Query("SELECT e from Sell e WHERE e.createdAt >= STR_TO_DATE(:start, '%Y-%m-%d %H:%i:%s') AND e.createdAt <= STR_TO_DATE(:end, '%Y-%m-%d %H:%i:%s') ")
