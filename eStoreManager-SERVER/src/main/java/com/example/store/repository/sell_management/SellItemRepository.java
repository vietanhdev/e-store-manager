...
public interface SellItemRepository extends JpaRepository<SellItem, Long> {
    ...
    @Query("SELECT new  com.example.store.repository.sell_management.BestSelling(SUM(e.quantities), e.product_id) FROM SellItem e WHERE e.createdAt >= STR_TO_DATE(:start, '%Y-%m-%d %H:%i:%s') AND e.createdAt <= STR_TO_DATE(:end, '%Y-%m-%d %H:%i:%s')  GROUP BY product_id ORDER BY SUM(e.quantities) DESC")
