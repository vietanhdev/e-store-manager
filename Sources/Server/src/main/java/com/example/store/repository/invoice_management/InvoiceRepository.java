...
public interface InvoiceRepository extends JpaRepository<Invoice, Long>{
    ...
    @Query("SELECT e FROM Invoice e WHERE e.purpose LIKE %:purpose% AND e.description LIKE %:description% AND ( e.purpose LIKE %:value% OR e.description LIKE %:value% ) AND ( e.createdAt >= STR_TO_DATE(:start, '%Y-%m-%d %H:%i:%s') AND e.createdAt <= STR_TO_DATE(:end, '%Y-%m-%d %H:%i:%s') ) ORDER BY e.createdAt DESC")
    ...
    @Query("SELECT e FROM Invoice e WHERE e.purpose LIKE %:purpose% AND e.description LIKE %:description% AND ( e.purpose LIKE %:value% OR e.description LIKE %:value% ) ORDER BY e.createdAt DESC")
